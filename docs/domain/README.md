...
このプロジェクトのドメインは Discord に依存します。

そのため、Guild、Channel、Thread、Message、Gateway Session、Sequence、Resume、Heartbeat、Intent、HTTP API、Snowflake ID などの Discord 固有概念はドメイン設計に直接含めて構いません。

一方、Durable Objects、Workers、Queues、R2 など、特定インフラストラクチャの仕組みはドメイン設計の主語にしません。

実際のインフラストラクチャ制約を踏まえてドメイン要件が変化することは許容します。その場合は、特定ベンダーの実装方法ではなく、DWH が満たすべき制約や保証として表現します。
...
- [lifecycle/README.md](lifecycle/README.md): retention、削除、長期的な data lifecycle
...
ドメイン設計は Canonical Store を特定 database の mutable current-state table として前提にしません。
