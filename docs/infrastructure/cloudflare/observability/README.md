...
本ディレクトリでは、Cloudflare 上で自律稼働する各コンポーネントの動作状況、健全性、および異常をリアルタイムに把握するためのログ収集、メトリクス集計、およびアラート通知のインフラ設計を定義する。

## 観測レイヤーと採用技術

Cloudflare 標準の可視化機能および外部連携を活用し、多層的なオブザーバビリティ基盤を整備する。
...
[運用アーキテクチャ](../../../architecture/cloudflare/operations/README.md) において定義された異常状態を検知した際、以下の通知ルートを通じて運用者へ迅速にエスカレーションする。
...
* **Logpush / Tail Workers の設定**: 本番ログを外部（Datadog, Grafana Cloud, または R2 自体）へ長期保管するための転送パイプラインの構築。
