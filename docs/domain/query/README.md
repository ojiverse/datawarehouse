...
メッセージ新規作成、複数回編集、リアクション追加、スレッドアーカイブ等の時系列観測イベント群から、**対象時点（または現時点）における適正状態を動的に導出する「プロジェクション（Projection）」**として最新状態を定義する。

### タイムトラベル（過去時点）分析の実現
プロジェクションによるアプローチを採用することで、「現時点の最新データ」のみならず、「過去の特定時点におけるサーバー状態」や「インシデント発生直前の会話ログ」など、過去の任意時点に遡った正確な状態復元クエリを可能とする。
